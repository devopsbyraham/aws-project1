...
NOTIFY_EMAIL="${NOTIFY_EMAIL:-devopsbyraham@gmail.com}"   # SRE alert email (optional but recommended)
...
# Robust Day-1 key discovery. Day 1 stores the key under ~/.ssh (or
# ~/.swiftcart-keys as a fallback), so never assume it is in $PWD.
KEY_CANDIDATES=(
  "${KEY_FILE:-}"
  "$HOME/.ssh/$KEY_PEM"
  "$HOME/.swiftcart-keys/$KEY_PEM"
  "$PWD/$KEY_PEM"
)
KEY_FILE=""
for _k in "${KEY_CANDIDATES[@]}"; do
  if [ -n "$_k" ] && [ -f "$_k" ]; then
    KEY_FILE="$(cd "$(dirname "$_k")" && pwd)/$(basename "$_k")"
    break
  fi
done
...
# SSH uses the exact Day-1 keypair discovered during preflight; never sudo the whole script.
# ─────────────────────────────────────────────────────────────────────────────
print_manual_docker() {
  cat <<MANUAL
...
do_docker_cutover() {
  if [ -z "${KEY_FILE:-}" ] || [ ! -f "$KEY_FILE" ]; then
    warn "Day-1 SSH key not found. Checked ~/.ssh, ~/.swiftcart-keys and $PWD."
    return 1
  fi
  [ -n "$BASTION_IP" ] && [ "$BASTION_IP" != "None" ] || { warn "No bastion public IP — skipping"; return 1; }
  [ -n "${WEB_ID:-}" ] && [ "$WEB_ID" != "None" ] || { warn "Web Portal instance not found — skipping"; return 1; }
  [ -n "${WEB_IP:-}" ] && [ "$WEB_IP" != "None" ] || { warn "Web Portal private IP not found — skipping"; return 1; }
  ...
  ok "IMDS hop limit set to 2 on the Web Portal"

  # Allow this workstation's current public IP to reach the Bastion.
  local myip sgb
  ...
  # Fail early with a precise SSH-authentication diagnosis instead of starting
  # the Docker cutover and then reporting a confusing publickey failure.
  log "Testing SSH authentication to Bastion with $KEY_FILE"
  if ! ssh -i "$KEY_FILE" -o BatchMode=yes -o StrictHostKeyChecking=accept-new \
      -o UserKnownHostsFile=/dev/null -o ConnectTimeout=15 \
      "ec2-user@$BASTION_IP" "echo BASTION_SSH_OK" >/tmp/swiftcart-bastion-ssh-test.out 2>/tmp/swiftcart-bastion-ssh-test.err; then
    warn "Bastion SSH authentication failed."
    cat /tmp/swiftcart-bastion-ssh-test.err >&2 || true
    warn "This means the key does not match the Bastion key pair, the key is not the Day-1 key, or ssh is using the wrong key."
    warn "Expected key locations: ~/.ssh/$KEY_PEM or ~/.swiftcart-keys/$KEY_PEM"
    return 1
  fi
  ok "Bastion SSH authentication works"

  # Test the complete ProxyCommand path to the private Web Portal before
  # mutating the host. This isolates jump-host/key problems.
  log "Testing SSH through Bastion to Web Portal"
  local proxy
  proxy="ssh -i $KEY_FILE -o BatchMode=yes -o StrictHostKeyChecking=accept-new -o UserKnownHostsFile=/dev/null -W %h:%p ec2-user@$BASTION_IP"
  if ! ssh -i "$KEY_FILE" -o BatchMode=yes -o StrictHostKeyChecking=accept-new \
      -o UserKnownHostsFile=/dev/null -o ConnectTimeout=20 \
      -o "ProxyCommand=$proxy" \
      "ec2-user@$WEB_IP" "echo WEBPORTAL_SSH_OK" >/tmp/swiftcart-web-ssh-test.out 2>/tmp/swiftcart-web-ssh-test.err; then
    warn "Web Portal SSH through Bastion failed."
    cat /tmp/swiftcart-web-ssh-test.err >&2 || true
    warn "Check SG-WebPortal inbound TCP/22 from SG-BastionHost and confirm the Web Portal uses the same key pair."
    return 1
  fi
  ok "Web Portal SSH through Bastion works"
...
echo "[INFO] Docker cutover starting on $(hostname)"
echo "[INFO] Region: $REG"
...
if ! command -v docker >/dev/null 2>&1; then
  if command -v amazon-linux-extras >/dev/null 2>&1; then
    ...
    echo "[ERROR] Cannot find an Amazon Linux package manager" >&2
    exit 1
  fi
fi
...
if ! command -v docker-compose >/dev/null 2>&1 && [ ! -x /usr/local/bin/docker-compose ]; then
  sudo curl -fsSL "https://github.com/docker/compose/releases/download/$DCVER/docker-compose-$(uname -s)-$(uname -m)" \
    -o /usr/local/bin/docker-compose
  sudo chmod +x /usr/local/bin/docker-compose
fi

mkdir -p ~/swiftcart_docker
cd ~/swiftcart_docker
...
REMOTE
  ...
  _msg_tty $'\n'"${C_CMD}\$ ssh -i \"$KEY_FILE\" -o ProxyCommand=\"ssh -i \"$KEY_FILE\" -W %h:%p ec2-user@$BASTION_IP\" ec2-user@$WEB_IP bash -s -- '$TOPIC_ARN' '$INV_IP' '$REPO_RAW' '$REGION' '$COMPOSE_VERSION'${C_OFF}"$'\n'
  _msg_tty "$remote_script"$'\n'
  _msg_tty "${C_CMD}REMOTE${C_OFF}"$'\n'
  if ! _consent; then
    _msg_tty "    (skipped by user)"$'\n'
    return 2
  fi

  log "Cutting Web Portal over to Docker"
  printf '%s\n' "$remote_script" | ssh -i "$KEY_FILE" \
    -o BatchMode=yes \
